 ...
 data SPDTMP7.VB_WB_OOS_LS_TXN_DV (Compress=yes);
      set spdtmp7.sd_txn_wb ;
  	   where TXN_DT_WID >= 20160201 and TXN_DT_WID<= 20160231 and LMG_CONCEPT_NAME = "Lifestyle" and TRANSACTION_TYPE = "Purchase" and units >= 0 and revenue_aed >= 0 and Retail_cost_1_AED >= 0 and Retail_cost_2_AED>= 0 ;
	   Bought_flag = 1;
 run;

proc sql ;
create table SPDTMP7.VB_WB_OOS_LS_Customers (compress=yes) as
select Distinct LMG_MEM_CARD_NUMBER, Bought_flag from SPDTMP7.VB_WB_OOS_LS_TXN_DV ;
quit;

proc sql;
create table spdtmp7.VB_WB_OOS_LS_Alldata13 (Compress = yes) as 
select a.*,b.*
from spdtmp7.VB_WB_OOS_LS_Alldata12 as a
left join SPDTMP7.VB_WB_OOS_LS_Customers as b
on a.LMG_MEM_CARD_NUMBER = b.LMG_MEM_CARD_NUMBER;
quit; 

Data spdtmp7.VB_WB_OOS_LS_Alldata14 (compress=yes);
set spdtmp7.VB_WB_OOS_LS_Alldata13;
...
proc stdize data=spdtmp7.VB_WB_OOS_LS_Alldata14 OUT=spdtmp7.VB_WB_OOS_LS_Alldata15 (compress=yes)  reponly missing=0;

run; 

proc freq data= spdtmp7.VB_WB_OOS_LS_Alldata15;
tables Bought_flag;
run;


proc delete data= spdtmp7.VB_WB_OOS_LS_Alldata11; run;
proc delete data= spdtmp7.VB_WB_OOS_LS_Alldata12; run;
proc delete data= spdtmp7.VB_WB_OOS_LS_Alldata13; run;
proc delete data= spdtmp7.VB_WB_OOS_LS_Alldata14; run;


/* Dividing data into Train and Test Datasets*/


Data spdtmp7.VB_WB_OOS_LS_Alldata16 (compress= yes drop= rnd_no);
set spdtmp7.VB_WB_OOS_LS_Alldata15;    
rnd_no = ranuni(123);
