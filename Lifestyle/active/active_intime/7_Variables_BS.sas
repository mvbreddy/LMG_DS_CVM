...
proc sql ;
  Create Table SPDTMP7.VB_BS_TN_ITM_REV_AE_1newer (compress = yes) as
   select LMG_MEM_CARD_NUMBER 
		...
		,('28Feb2017'd-max(case when Visit_flag=1 or Visit_flag = 2 or Visit_flag = 3 then date else . end))as recency_9_BS

		, (max(date)-min(date)) as period_bw_trnxns_BS
		
from SPDTMP7.VB_BS_TN_ITM_REV_AE_
...
data SPDTMP7.VB_BS_TN_ITM_REV_AE_1_1(compress = yes);
set SPDTMP7.VB_BS_TN_ITM_REV_AE_1newer;
