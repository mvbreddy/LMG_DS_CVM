...
/*Creating a dataset of final Variables from Table VB_LS_TN_ITM_REV_AE i.e. Data of only the Lifestyle concept*/
/*Joining the DIM_ITM_LS Table to LS Transactions table */

data SPDTMP7.VB_LS_Dim_itm_ls (compress= yes) ;
set saser.dim_itm_ls;
run;

proc sql;
  create table SPDTMP7.VB_LS_TN_ITM_REV_AE_0 (compress = yes) as
  select t1.* , t2.GRP_CD , t2.GRP_NM , t2.DPT_CD, DPT_NM from SPDTMP7.VB_LS_TN_ITM_REV_AE_ as t1
  left join SPDTMP7.VB_LS_Dim_itm_ls as t2
...
proc sql ;
  Create Table SPDTMP7.VB_LS_TN_ITM_REV_AE_1 (compress = yes) as
   select LMG_MEM_CARD_NUMBER 
		...
		,('28Feb2017'd- max(case when Visit_flag=1 or  Visit_flag = 2 then date else . end))as recency_6
		,('28Feb2017'd- max(case when Visit_flag=1 or  Visit_flag = 2 or Visit_flag = 3 then date else . end))as recency_9
...
from SPDTMP7.VB_LS_TN_ITM_REV_AE_0
Group By LMG_MEM_CARD_NUMBER ;
quit;



data SPDTMP7.VB_LS_TN_ITM_REV_AE_1_1 (compress=yes);
set SPDTMP7.VB_LS_TN_ITM_REV_AE_1;
