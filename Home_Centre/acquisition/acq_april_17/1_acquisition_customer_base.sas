...
proc sql;
create table spdtmp7.sd_txn_custnonhc16_apr (compress=yes) as 
...
/*2years*/
proc sql;
create table spdtmp7.sd_txn_custnonhc1516_apr (compress=yes) as 
select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where txn_dt_wid>=20150401 and txn_dt_wid<=20170331 and 
Lmg_mem_card_number not in (select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where Lmg_concept_name="Home center" and txn_dt_wid>=20150401  and txn_dt_wid<=20170331 );
quit;

data spdtmp7.sd_hc_txn_ac_cust_apr(compress=yes);
merge spdtmp7.sd_txn_custnonhc1516_apr(in=b) spdtmp7.sd_txn_custnonhc16_apr(in=a);
