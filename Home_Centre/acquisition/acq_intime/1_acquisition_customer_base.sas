...
proc sql;
create table spdtmp7.sd_txn_custnonhc15 (compress=yes) as 
select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where txn_dt_wid>=20150301 and txn_dt_wid<=20160229 and 
...
proc sql;
create table spdtmp7.sd_txn_custnonhc16 (compress=yes) as 
select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where txn_dt_wid>=20160301 and txn_dt_wid<=20170228 and 
Lmg_mem_card_number not in (select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where Lmg_concept_name="Home center" and txn_dt_wid>=20160301  and txn_dt_wid<=20170228 );
quit;

/*2years*/
proc sql;
create table spdtmp7.sd_txn_custnonhc1516 (compress=yes) as 
select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where txn_dt_wid>=20150301 and txn_dt_wid<=20170228 and 
Lmg_mem_card_number not in (select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where Lmg_concept_name="Home center" and txn_dt_wid>=20150301  and txn_dt_wid<=20170228 );
quit;

data spdtmp7.sd_hc_txn_ac_cust(compress=yes);
merge spdtmp7.sd_txn_custnonhc1516(in=b) spdtmp7.sd_txn_custnonhc16(in=a);
...
select count(distinct LMG_mem_Card_number) from spdtmp7.sd_hc_txn_ac_cust ;
quit;
