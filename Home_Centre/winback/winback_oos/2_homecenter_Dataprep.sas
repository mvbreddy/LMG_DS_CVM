proc sql;
create table spdtmp7.sd_txn_wboos_all(compress=yes) as
select a.*,b.* from spdtmp7.sd_hc_txn_wb_custoos a left join spdtmp7.sd_txn_wb b
on a.LMG_mem_Card_number=b.LMG_mem_card_number;
quit;

/*Home center winback customers*/

data spdtmp7.sd_hc_txn_wboos(compress=yes keep=lmg_mem_card_number txn_dt_wid lmg_x_dept_cd revenue_aed invoice_number units item_code base_points_accrued retail_cost_1_aed retail_cost_2_aed date);
set spdtmp7.sd_txn_wboos_all;
where txn_dt_wid>=20140301 and txn_dt_wid<=20150228 and LMG_concept_name="Home center" and revenue_aed>0 and units>0 and base_points_accrued >0 ;
...
select count(distinct LMG_mem_Card_number), max(txn_dt_wid) from spdtmp7.sd_hc_txn_wboos;
quit;
...
recency=intck('day',end_date,'28Feb2015'd);
...
proc sql;
Create Table spdtmp7.sd_hc_txn_wboos (compress=yes) as 
select a.*,b.SEX_MF_NAME,b.AGE,b.LMG_EFFECTIVE_POINTS,b.CVM_Nationality_group from spdtmp7.sd_hc_txn_wboos a
