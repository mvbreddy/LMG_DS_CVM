...
select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb where
 txn_dt_wid>=20160301 and  txn_dt_wid<=20170228 and Lmg_mem_card_number not in (select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
...
data spdtmp7.sd_hc_txn_wb_cust(compress=yes);
