...
data spdtmp7.sd_hc_txn_all (compress=y);
set sascrm.tn_itm_rev_ae ;
where txn_dt_wid>=20160301 and txn_dt_wid<=20170228 and Lmg_concept_name="Home center";
run;
/*subsetting data*/
data spdtmp7.sd_hc_txn_all1(compress=y);
set spdtmp7.sd_hc_txn_all (keep= lmg_mem_card_number txn_dt_wid revenue_aed invoice_number units item_code base_points_accrued retail_cost_1_aed retail_cost_2_aed);
where units >0 and revenue_aed >0 and base_points_accrued >0;
date = INPUT(PUT(TXN_DT_WID,8.),YYMMDD8.); 
           format date ddmmyys10.;

run;
proc contents data=spdtmp7.sd_hc_txn_all;
run;


proc sql;
create table spdtmp7.sd_hc_txn_all2 (compress=yes) as
...
data spdtmp7.sd_hc_txn_all3(compress=yes);
set spdtmp7.sd_hc_txn_all2;
format start_date ddmmyys10.;
format end_date ddmmyys10.;
recency=intck('day',end_date,'28Feb2017'd);
...
data spdtmp7.sd_hc_cust (Compress=yes keep= LMG_MEM_CARD_NUMBER SEX_MF_NAME  
...
proc sql;
Create Table spdtmp7.sd_hc_txn_cust (compress=yes) as 
select a.*,b.* from spdtmp7.sd_hc_txn_all3 a
left join spdtmp7.sd_hc_cust b
