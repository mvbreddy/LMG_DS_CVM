...
proc sql;
create table spdtmp7.sd_txn_acmay_all(compress=yes) as
select a.*,b.* from spdtmp7.sd_hc_txn_ac_cust_may a left join spdtmp7.sd_txn_wb b
on a.LMG_mem_Card_number=b.LMG_mem_card_number;
quit;

/*acq customers*/

data spdtmp7.sd_hc_txn_acmay(compress=yes keep=lmg_mem_card_number txn_dt_wid lmg_x_dept_cd revenue_aed invoice_number units item_code base_points_accrued retail_cost_1_aed retail_cost_2_aed date);
...
proc sql;
Create Table spdtmp7.sd_hc_txn_acmay (compress=yes) as 
select a.*,b.SEX_MF_NAME,b.AGE,b.LMG_EFFECTIVE_POINTS,b.CVM_Nationality_group from spdtmp7.sd_hc_txn_acmay a
...
merge spdtmp7.sd_hc_txn_acmay_cust(in=a) spdtmp7.sd_hc_txn_wb_cust(in=b);
if a=1 and b=1 ;
