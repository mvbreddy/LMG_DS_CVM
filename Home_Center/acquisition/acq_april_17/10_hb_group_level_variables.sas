...
proc sql;
create table spdtmp7.sd_hc_txn_acapr1 as
select a.*,b.grp_nm from spdtmp7.sd_hc_txn_acapr1 a inner join saser.dim_itm_hb b on a.item_code=b.itm_cd;
run; 


proc freq data=spdtmp7.sd_hc_txn_acapr1;
tables grp_nm;
run;
...
from spdtmp7.sd_hc_txn_acapr1 group by LMG_mem_card_number;
quit;
...
select a.*,b.* from spdtmp7.sd_hc_txn_acapr a left join  spdtmp7.sd_hc_txn_acapr2 b on a.LMG_mem_card_number=b.LMG_mem_card_number
;
quit; 

data spdtmp7.sd_hc_txn_acapr(compress=yes drop=revenue_AED_furhb 
revenue_Aed_hwhb 
revenue_Aed_kidshb 
revenue_Aed_mhnthb 
total_units_furhb 
