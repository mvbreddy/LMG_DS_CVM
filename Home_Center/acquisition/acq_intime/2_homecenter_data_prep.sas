...
proc sql;
create table spdtmp7.sd_txn_ac_all(compress=yes) as
...
data spdtmp7.sd_hc_txn_ac(compress=yes keep=lmg_mem_card_number txn_dt_wid lmg_x_dept_cd revenue_aed invoice_number units item_code base_points_accrued retail_cost_1_aed retail_cost_2_aed date);
...
select count(distinct LMG_mem_Card_number), max(txn_dt_wid) from spdtmp30.sd_hc_txn_ac;
quit;
...
proc sql;
Create Table spdtmp7.sd_hc_txn_ac (compress=yes) as 
select a.*,b.SEX_MF_NAME,b.AGE,b.LMG_EFFECTIVE_POINTS,b.CVM_Nationality_group from spdtmp7.sd_hc_txn_ac a
...
merge spdtmp7.sd_hc_txn_ac_cust(in=a) spdtmp7.sd_hc_txn_wb_cust(in=b);
if a=1 and b=1 ;
