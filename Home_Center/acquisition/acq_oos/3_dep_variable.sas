...
where txn_dt_wid>=20160301 and txn_dt_wid<=20160331 and LMG_concept_name="Home center" and revenue_aed>0 and units>0 and base_points_accrued >0 ;
...
select distinct LMG_mem_card_number from spdtmp7.sd_hc_txn_acoos1
...
select a.*,b.* from spdtmp7.sd_hc_txn_acoos a left join spdtmp7.sd_hc_txn_acoos1 b on
...
data spdtmp7.sd_hc_txn_acoos(compress=yes drop=purchase_flag);
set spdtmp7.sd_hc_txn_acoos;
run;