...
proc sql;
create table spdtmp7.sd_txn_custnonhc14 (compress=yes) as 
select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where txn_dt_wid>=20140301 and txn_dt_wid<=20150228 and 
...
proc sql;
create table spdtmp7.sd_txn_custnonhc1415 (compress=yes) as 
select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where txn_dt_wid>=20140301 and txn_dt_wid<=20160229 and 
Lmg_mem_card_number not in (select distinct LMG_mem_Card_number from spdtmp7.sd_txn_wb
where Lmg_concept_name="Home center" and txn_dt_wid>=20140301 and txn_dt_wid<=20160229 );
quit;

data spdtmp7.sd_hc_txn_ac_cust_oos(compress=yes);
merge spdtmp7.sd_txn_custnonhc14(in=a) spdtmp7.sd_txn_custnonhc1415(in=b);
...
select count(distinct LMG_mem_Card_number) from spdtmp7.sd_hc_txn_ac_cust_oos;
quit;
...
data x(compress=yes);
merge spdtmp7.sd_hc_txn_final6_oos(in=a) spdtmp7.sd_hc_txn_ac_cust(in=b) spdtmp7.sd_hc_txn_wb_cust(in=c);
