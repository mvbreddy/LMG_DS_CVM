...
data spdtmp7.sd_hc_txn_acoos1(compress=yes keep=lmg_mem_card_number txn_dt_wid lmg_x_dept_cd revenue_aed invoice_number units item_code base_points_accrued retail_cost_1_aed retail_cost_2_aed date);
...
proc sql;
create table spdtmp7.sd_hc_txn_acoos1 as
select a.*,b.grp_nm from spdtmp7.sd_hc_txn_acoos1 a inner join saser.dim_itm_hb b on a.item_code=b.itm_cd;
run; 


proc freq data=spdtmp7.sd_hc_txn_acoos1;
tables grp_nm;
run;
...
from spdtmp7.sd_hc_txn_acoos1 group by LMG_mem_card_number;
quit;
...
select a.*,b.* from spdtmp7.sd_hc_txn_acoos a left join  spdtmp7.sd_hc_txn_acoos2 b on a.LMG_mem_card_number=b.LMG_mem_card_number
;
quit; 