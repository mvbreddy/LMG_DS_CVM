data spdtmp7.sd_hc_txn_final6_may(compress=yes keep=lmg_mem_card_number txn_dt_wid lmg_x_dept_cd revenue_aed invoice_number units item_code base_points_accrued retail_cost_1_aed retail_cost_2_aed date);
...
data spdtmp7.sd_hc_txn_final6_may(compress=yes);

set spdtmp7.sd_hc_txn_final6_may;
...
proc sql;
create table spdtmp7.sd_hc_txn_final6_may(compress=yes) as
...
from spdtmp7.sd_hc_txn_final6_may group by LMG_mem_card_number;
quit;

proc sql;
create table spdtmp7.sd_hc_activemay(compress=yes) as
select a.*,b.* from spdtmp7.sd_hc_activemay a left join  spdtmp7.sd_hc_txn_final6_may b on a.LMG_mem_card_number=b.LMG_mem_card_number
;
quit;