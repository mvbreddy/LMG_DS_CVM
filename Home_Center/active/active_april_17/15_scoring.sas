...
data spdtmp7.sd_hc_txn_final6_apr(compress=yes);
   set spdtmp7.sd_hc_txn_final6_apr;
...
from spdtmp7.sd_hc_txn_final6_apr;
quit;
...
proc logistic  
     ...
     score data = spdtmp7.sd_hc_txn_final6_apr
     Out  = spdtmp7.sd_hc_score_april_active;
run; 



data spdtmp7.sd_hc_model_confusion;
set spdtmp7.sd_hc_score_april_active;

if P_1 >= 0.11 then prediction = 1;
else prediction = 0;
run;
