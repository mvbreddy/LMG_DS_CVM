data spdtmp7.sd_hc_txn_grp(compress=yes);
...
data spdtmp7.sd_hc_txn_grp1(compress=yes);

set spdtmp7.sd_hc_txn_grp;
where units >0 and revenue_aed >0;
...
proc freq data=spdtmp7.sd_hc_txn_grp1;
tables lmg_x_dept_cd;
run;

data spdtmp7.sd_hc_txn_grp2(compress=yes);
set spdtmp7.sd_hc_txn_grp1;
where hc_group="Furniture" or hc_group="kids" or hc_group="Household";
run;


proc sql;
create table spdtmp7.sd_hc_txn_grp3(compress=yes) as
...
from spdtmp7.sd_hc_txn_grp2 group by LMG_mem_card_number;
quit;
...
proc sql;
create table spdtmp7.sd_hc_txn_grp4(compress=yes) as 
select LMG_mem_card_number,
...
proc sql;
create table spdtmp7.sd_hc_txn_final3(compress=yes) as
select a.*,b.*,c.* from spdtmp7.sd_hc_txn_final2 a left join  spdtmp7.sd_hc_txn_grp3 b on a.LMG_mem_card_number=b.LMG_mem_card_number
left join  spdtmp7.sd_hc_txn_grp4 c on
a.LMG_mem_card_number=c.LMG_mem_card_number;
quit; 

proc delete data=spdtmp7.sd_hc_txn_final2;
run;

proc means data=spdtmp7.sd_hc_txn_final3;
run;
