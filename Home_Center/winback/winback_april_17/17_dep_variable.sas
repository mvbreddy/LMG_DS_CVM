...
select distinct LMG_mem_card_number from spdtmp7.sd_hc_txn_wbapr1
...
select a.*,b.* from spdtmp7.sd_hc_txn_wbapr a left join spdtmp7.sd_hc_txn_wbapr1 b on
...
data spdtmp7.sd_hc_txn_wbapr(compress=yes drop=purchase_flag);
set spdtmp7.sd_hc_txn_wbapr;
run;