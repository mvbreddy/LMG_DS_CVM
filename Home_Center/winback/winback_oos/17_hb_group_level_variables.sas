...
where txn_dt_wid >=20140301 and txn_dt_wid<=201502298and LMG_concept_name="Home Box" and revenue_aed>0 and units>0 and base_points_accrued >0 ;
...
proc sql;
create table spdtmp7.sd_hc_txn_wboos1 as
select a.*,b.grp_nm from spdtmp7.sd_hc_txn_wboos1 a inner join saser.dim_itm_hb b on a.item_code=b.itm_cd;
run; 
