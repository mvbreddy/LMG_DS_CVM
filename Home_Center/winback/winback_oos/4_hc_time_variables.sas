...
where txn_Dt_wid>=20141201 and txn_dt_wid<=20150228 and LMG_concept_name="Home center" and revenue_aed>0 and units>0 and base_points_accrued >0 ;
...
proc sql;
select count(distinct LMG_mem_card_number) from spdtmp7.sd_hc_txn_wboos1;
quit;
...
recency_3=intck('day',end_date_3,'28Feb2015'd);
...
data spdtmp7.sd_hc_txn_wboos2(compress=yes keep=lmg_mem_card_number txn_dt_wid lmg_x_dept_cd revenue_aed invoice_number units item_code base_points_accrued retail_cost_1_aed retail_cost_2_aed date);
set spdtmp7.sd_txn_wboos_all;
where txn_Dt_wid>=20140901 and txn_dt_wid<=20150228 and LMG_concept_name="Home center" and revenue_aed>0 and units>0 and base_points_accrued >0 ;
date = INPUT(PUT(TXN_DT_WID,8.),YYMMDD8.); 
           format date ddmmyys10.;
run;
...
recency_6=intck('day',end_date_6,'28Feb2015'd);
...
data spdtmp7.sd_hc_txn_wboos3(compress=yes keep=lmg_mem_card_number txn_dt_wid lmg_x_dept_cd revenue_aed invoice_number units item_code base_points_accrued retail_cost_1_aed retail_cost_2_aed date);
set spdtmp7.sd_txn_wboos_all;
where txn_Dt_wid>=20140601 and txn_dt_wid<=20150228 and LMG_concept_name="Home center" and revenue_aed>0 and units>0 and base_points_accrued >0 ;
date = INPUT(PUT(TXN_DT_WID,8.),YYMMDD8.); 
           format date ddmmyys10.;
run;
...
recency_9=intck('day',end_date_9,'28Feb2015'd);
...
select a.*,b.*,c.*,d.* from spdtmp7.sd_hc_txn_wboos a left join  spdtmp7.sd_hc_txn_wboos1 b on a.Lmg_mem_card_number=b.LMG_mem_card_number
left join spdtmp7.sd_hc_txn_wboos2 c on a.LMG_mem_card_number=c.LMG_mem_card_number
left join spdtmp7.sd_hc_txn_wboos3 d on a.LMG_mem_card_number=d.LMG_mem_card_number;
quit;
...
proc delete data=spdtmp7.sd_hc_txn_wboos2;
proc delete data=spdtmp7.sd_hc_txn_wboos3;